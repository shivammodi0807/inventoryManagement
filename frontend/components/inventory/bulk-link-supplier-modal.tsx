...
export function BulkLinkSupplierModal({ 
  ...
  const {
    ...
    resolver: zodResolver(bulkLinkSchema) as any,
    defaultValues: {
      supplier_id: 0,
      est_delivery_days: 7,
      min_order_qty: 1,
  ...
  const onSubmit = async (values: BulkLinkFormValues) => {
      ...
      try {
        await linkMutation.mutateAsync({
          ...
          data: {
            ...
            cost_price: "0.00", // Bulk linking usually needs manual cost entry later or a default
          },
        });
        successCount++;
      } catch (err) {
    ...
    setIsProcessing(false);
    if (failCount === 0) {
      toast.success(`Successfully linked ${successCount} products to the supplier.`);
      onOpenChange(false);
      onSuccess?.();
    } else {
      ...
      <DialogContent className="sm:max-w-[500px]">
        <form onSubmit={handleSubmit(onSubmit as any)}>
          <DialogHeader>
            ...
            <DialogDescription>
              Associate {productIds.length} selected products with a single supplier.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <Alert variant="warning" className="bg-amber-50 border-amber-200 text-amber-800">
              ...
              <AlertDescription className="text-xs">
                Cost prices will be set to 0.00. You can update individual costs in the product or supplier details page later.
              </AlertDescription>
            </Alert>
