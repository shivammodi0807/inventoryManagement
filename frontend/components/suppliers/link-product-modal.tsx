...
export function LinkProductModal({ supplierId, open, onOpenChange }: LinkProductModalProps) {
  ...
  // Fetch products (for a real app with many products, this would be an infinite query or server-filtered)
  const { data: productsData, isLoading: isLoadingProducts } = useQuery({
    queryKey: ["products", "all"],
    queryFn: () => getProducts({ per_page: 100 }), // Get up to 100 products for now
  });
